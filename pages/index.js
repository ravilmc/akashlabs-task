...
export default function Home() {
  ...
  useEffect(() => {
    ...
    setLoading(true)
    const getcurrencyurl =zabobaseurl+'/currencies'
      ...
      {loading ? (
        ...
        <div className={styles.table}>
          <div className={styles.table_header}>
            {headers.map((header) => (
              <div className={styles.table_header_title} key={header}>{header}</div>
            ))}
          </div>
          <div className={styles.table_list_wrapper}>
            {currencies.map((currency ,index) => (
              <CryptoListItem data={currency} loadData={loadData} key={index+currency.ticker } />
            ))}
